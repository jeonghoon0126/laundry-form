...
## 숙소 (운영 10개)
장충동(메종드브릭) / 제기동(스테이브리즈) / 건대(화양프라하임) / 회기(오를리) / 청량리(롯데캐슬) / 강남(신규 숙소, 건물 앞 정차 권장, 5층 엘리베이터 옆 수납창고) / 송파(스테이브리즈) / 신림(스테이모먼트) / 연남(에코리빙) / 이태원(서울특별시 용산구 회나무로 50, 5층 엘리베이터 진입 후 반층 위 렉 설치 예정) / 은평(은평구 통일로 863-10, 정문현관 - 5052*, 엘리베이터 없음, 1층 세탁물 보관)
