...
- 2026-08-12부터 매주 수요일은 `장충단로 225` 한 곳만 수거·배송 동선에 포함
...
- GitHub schedule은 월·수·목 `10:00 KST` 기준으로 잡혀 있지만 실제 실행은 지연될 수 있다.
...
## 숙소 (운영 9개)
장충동(메종드브릭) / 제기동(스테이브리즈) / 건대(화양프라하임) / 회기(오를리) / 청량리(롯데캐슬) / 강남(강남 언주로 숙소, 서울 강남구 봉은사로37길 8, 5층 엘리베이터 옆 수납창고) / 송파(스테이브리즈) / 신림(스테이모먼트) / 연남(에코리빙) / 이태원(서울특별시 용산구 회나무로 50, 최신 출입 안내는 동선 문자 코드 기준)
